...
func _Key_and_Goals_setup():
	for idx in range(3,-1,-1):
		var node = _add_child($Display/Registers/Key, TextureRect.new(), "K%s" % idx)
		node.set_texture(load("res://assets/B%s.png" % 2**idx))
		_TextureRect_setup(node)

	var node = _add_child($Display/Registers/Key, TextureButton.new(), "Goal")
...
func _Registers_setup():
	for id in ["A", "B", "C", "D"]:
		var node = _add_child($Display/Registers, Register.new(), "R%s" % id)
